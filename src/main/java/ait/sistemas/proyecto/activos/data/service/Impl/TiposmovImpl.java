...
public class TiposmovImpl implements Dao<Tipos_Movimiento> {
	...
	public List<Tipos_Movimiento> getall() {
		Query query = em.createNativeQuery("Para_Tiposmov_Q ",
				Tipos_Movimiento.class).setHint(QueryHints.REFRESH,
				HintValues.TRUE);
	...
	public Tipos_Movimiento add(Tipos_Movimiento table) {
		...
		try {
			Tipos_Movimiento result = (Tipos_Movimiento) query
					.getSingleResult();
			return result;
		} catch (Exception e) {
			return null;
	...
	public Tipos_Movimiento update(Tipos_Movimiento table) {
		String strQuery = String
				.format("EXEC Para_Tiposmov_U " + "@TMV_Tipo_Movimiento=?1, "
						+ "@TMV_Nombre_Tipo_Movimiento=?2, "
						+ "@TMV_Sigla_Tipo_Movimiento=?3, "
						+ "@TMV_Fecha_Registro=?4 ");
		Query query = this.em.createNativeQuery(strQuery,
