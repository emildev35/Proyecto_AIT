...
public class PersonalImpl implements Dao<Personal> {
	...
	public List<PersonalModel> getalls() {
		...
		Query query = em.createNativeQuery("exec Rrhh_Personal_Q",
				"archive-map-p").setHint(QueryHints.REFRESH, HintValues.TRUE);
	...
	public Personal add(Personal table) {
		String strQuery = String.format("EXEC Rrhh_Personal_I "
				+ "@PER_Dependencia=?1, " + "@PER_Unidad_Organizacional=?2, "
				+ "@PER_CI_Empleado=?3, " + "@PER_Apellido_Paterno=?4, "
				+ "@PER_Apellido_Materno=?5, " + "@PER_Nombres=?6, "
				+ "@PER_No_Telefono_Oficina=?7, " + "@PER_No_Interno=?8, "
