...
public class GrupoImpl implements Dao<Grupos_Contable> {
	...
	public List<GruposContablesModel> getalls() {
		...
		Query query = em.createNativeQuery("exec Para_Grupo_Q", "archive-map-g").setHint(QueryHints.REFRESH, HintValues.TRUE);
		List<GruposContablesModel> resultlist = query.getResultList();		
		return resultlist;
	}
	public List<GruposContablesModel> getpartida(int id_partida) {
		Query query = em.createNativeQuery("Para_Grupo_Partida "
				+ "@GRC_Partida=?1 ", GruposContablesModel.class);
	...
	public Grupos_Contable add(Grupos_Contable table) {
		String strQuery = String.format("EXEC Para_Grupo_I "
				+ "@GRC_Grupo_Contable=?1, "
				+ "@GRC_Nombre_Grupo_Contable=?2, "
				+ "@GRC_Vida_Util=?3, "
				+ "@GRC_Coeficiente=?4, "
				+ "@GRC_Partida=?5, "
				+ "@GRC_Fecha_Registro=?6");
	...
	Query query = em.createNativeQuery("Para_Grupo_D "
			+ "@GRC_Grupo_Contable=?1 ");
	...
	public Grupos_Contable update(Grupos_Contable table) {
		String strQuery = String.format("EXEC Para_Grupo_U "
				+ "@GRC_Grupo_Contable=?1, "
				+ "@GRC_Nombre_Grupo_Contable=?2, "
				+ "@GRC_Vida_Util=?3, "
				+ "@GRC_Coeficiente=?4, "
