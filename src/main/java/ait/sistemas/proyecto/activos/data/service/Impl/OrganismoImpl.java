...
public class OrganismoImpl implements Dao<Organismo_Financiador> {
	...
	public List<Organismo_Financiador> getall() {
		Query query = em.createNativeQuery("Para_Organismo_Q ", Organismo_Financiador.class).setHint(QueryHints.REFRESH,
				HintValues.TRUE);
	...
	public Organismo_Financiador getone(long id) {
		Query query = em.createNativeQuery("Para_Organismo_UQ  @Id_Organismo=?1", Organismo_Financiador.class).setHint(QueryHints.REFRESH,
				HintValues.TRUE).setParameter(1, id);
		Organismo_Financiador resultlist = (Organismo_Financiador)query.getSingleResult();
		return resultlist;
	}
	
	@Override
	public Organismo_Financiador add(Organismo_Financiador table) {
		String strQuery = String.format("EXEC Para_Organismo_I " + "@ORF_Organismo_Financiador=?1, "
				+ "@ORF_Nombre_Organismo_Financiador=?2, " + "@ORF_Fecha_Registro=?3 ");
	...
	public int deletes(short id_organismo) {
		Query query = em.createNativeQuery("Para_Organismo_D " + "@ORF_Organismo_Financiador=?1 ");
	...
	public Organismo_Financiador update(Organismo_Financiador table) {
		String strQuery = String.format("EXEC Para_Organismo_U " + "@ORF_Organismo_Financiador=?1, "
				+ "@ORF_Nombre_Organismo_Financiador=?2, " + "@ORF_Fecha_Registro=?3 ");
		Query query = this.em.createNativeQuery(strQuery, Organismo_Financiador.class);
		query.setParameter(1, table.getORF_Organismo_Financiador());
		query.setParameter(2, table.getORF_Nombre_Organismo_Financiador());
