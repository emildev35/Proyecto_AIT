...
public class AuxiliarImpl implements Dao<Auxiliares_Contable> {
	...
	public List<AuxiliaresContablesModel> getreporte(String id_grupo) {
		Query query = em.createNativeQuery("Para_Auxiliar_Grupo " + "@AUC_Grupo_Contable=?1 ", AuxiliaresContablesModel.class);
		query.setParameter(1, id_grupo);
		List<AuxiliaresContablesModel> resultlist = query.getResultList();
		return resultlist;
	}
	
	public AuxiliaresContablesModel get(String idAux, String idGrupo) {
		Query query = em.createNativeQuery("exec Para_Auxiliar_UQ @id_aux=?1, @id_grup=?2", "archive-map-a")
	...
	public Auxiliares_Contable add(Auxiliares_Contable table) {
		String strQuery = String.format("EXEC Para_Auxiliar_I " + "@AUC_Grupo_Contable=?1, " + "@AUC_Auxiliar_Contable=?2, "
				+ "@AUC_Nombre_Auxiliar_Contable=?3, " + "@AUC_Fecha_Registro=?4 ");
	...
	public int delete(String id_grupo, String id_auxiliar) {
		Query query = em.createNativeQuery("Para_Auxiliar_D " + "@AUC_Grupo_Contable=?1," + "@AUC_Auxiliar_Contable=?2");
		query.setParameter(1, id_grupo);
	...
	public Auxiliares_Contable update(Auxiliares_Contable table) {
		String strQuery = String.format("EXEC Para_Auxiliar_U " + "@AUC_Grupo_Contable=?1, " + "@AUC_Auxiliar_Contable=?2, "
				+ "@AUC_Nombre_Auxiliar_Contable=?3, " + "@AUC_Fecha_Registro=?4 ");
		Query query = this.em.createNativeQuery(strQuery, Auxiliares_Contable.class);
		query.setParameter(1, table.getAUC_Grupo_Contable());
		query.setParameter(2, table.getAUC_Auxiliar_Contable());
