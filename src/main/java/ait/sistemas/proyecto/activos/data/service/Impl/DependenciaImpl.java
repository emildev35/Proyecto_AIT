...
public class DependenciaImpl implements Dao<Dependencia> {
	...
	public List<Dependencia> getall() {
		...
		Query query = em.createNativeQuery("exec Rrhh_Dependencia_Q", Dependencia.class).setHint(
	...
	public List<Dependencia> getdependencia_ciudad(short padre) {
		...
		Query query = em.createNativeQuery("exec Rrhh_Dependencia_Ciudad_Q " + "@DEP_Ciudad=?1",
...
//		return resultlist;
	...
	public Dependencia add(Dependencia table) {
		String strQuery = String.format("EXEC Rrhh_Dependencia_I " + "@DEP_Dependencia=?1, "
				+ "@DEP_Nombre_Dependencia=?2, " + "@DEP_Sigla=?3, " + "@DEP_Ciudad=?4, "
				+ "@DEP_Domicilio=?5, " + "@DEP_Pagina_Web=?6, " + "@DEP_Telefono=?7, "
				+ "@DEP_Fecha_Registro=?8");
		Query query = this.em.createNativeQuery(strQuery, Dependencia.class);
	...
	public int deletes(int id_dependencia) {
		String strQuery = String.format("EXEC Rrhh_Dependencia_D " + "@DEP_Dependencia=?1");
	...
	public Dependencia update(Dependencia table) {
		String strQuery = String.format("EXEC Rrhh_Dependencia_U " + "@DEP_Dependencia=?1, "
				+ "@DEP_Nombre_Dependencia=?2, " + "@DEP_Sigla=?3, " + "@DEP_Ciudad=?4, "
				+ "@DEP_Domicilio=?5, " + "@DEP_Pagina_Web=?6, " + "@DEP_Telefono=?7, "
				+ "@DEP_Fecha_Registro=?8");
		Query query = this.em.createNativeQuery(strQuery, Dependencia.class);
