...
public class GridActaTrans extends Grid{
	...
	public void buildGrid(String ci_funcionario){
		bean_Movimiento = new BeanItemContainer<Movimiento>(Movimiento.class);
		bean_Movimiento.addAll(movimiento_impl.getActaDbyFuncionario(ci_funcionario, (short)6));
		setContainerDataSource(bean_Movimiento);
		setHeightMode(HeightMode.ROW);
		setHeightByRows(8);
		setWidth("100%");
...
//		getColumn("fecha_movimiento").setHeaderCaption("No Acta");
		...
		no_solicitudColumn.setRenderer(new NumberRenderer(new DecimalFormat("###")));
