...
public class VDevolucionA extends VerticalLayout implements View, ClickListener,
	...
	private FormDevolucion frm_devolucion;
	...
	public VDevolucionA() {
		...
		this.frm_devolucion = new FormDevolucion();
	...
	//	formContent.setMargin(true);
		...
		formContent.addComponent(this.frm_devolucion);
	...
	public void select(SelectionEvent event) {
		...
		if ((Movimiento) this.grid_asignacion.getSelectedRow() != null) {
			this.frm_devolucion.setData((Movimiento) 
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton() == this.btn_asignacion) {
			if (this.frm_devolucion.validate()) {
				this.acta_impl.addActaTransferencia(this.frm_devolucion.getData());
				this.grid_Detalle.vaciar();
				this.frm_devolucion.update();
				this.frm_devolucion.buidId();
			...
			buildMessages(this.frm_devolucion.getMensajes());
			this.frm_devolucion.clearMessages();
		}
		if (event.getButton() == this.btn_imprimir) {
			ReportPdf reporte = new ReportPdf();
			List<MovimientoReporte> data_reporte = acta_impl.ReporteActa(frm_devolucion.txt_no_acta.getValue(),
					(short) 4);
			try {
				reporte.getPdf(getActa(data_reporte), Short.parseShort(frm_devolucion.txt_no_acta.getValue()),
						(short) 4);
			...
			Window subWindow = new Window("Reporte Acta Devolucion");
