...
import ait.sistemas.proyecto.activos.view.inve.activosxgrupo.reporte.PDFInventarioGenerator;
...
public class ReportPdf {
	...
	public boolean getPdf(String[][] data, String strDependencia, String fecha, String grupo) throws IOException {
	...
	private Table createContent(String[][] content, String strDependencia, String fecha) {
		...
		columns.add(new Column("Fecha Incorporacion", 65));
		...
		Table Inventario = new TableBuilder().setCellMargin(CELL_MARGIN).setColumnsGA(columnsGA).setContent(content)
				...
				.setTitle("ACTIVOS POR GRUPO CONTABLE").setSubTitle("Realizado al : " + fecha).build();
		return Inventario;
