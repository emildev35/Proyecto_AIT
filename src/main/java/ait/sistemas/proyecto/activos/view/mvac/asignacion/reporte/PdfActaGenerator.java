...
public class PdfActaGenerator {
	...
	private void drawCurrentPage(Acta table, PDPageContentStream contentStream, int pageCount, String[][] data)
		...
		if (pageCount == 0) {
			tableTopY = table.isLandscape() ? table.getPageSize().getWidth() - table.getMargin()
					- (table.getRowHeight() * table.getHeaderSize()) : table.getHeight()
					- table.getMargin()  - (table.getRowHeight() * table.getHeaderSize());
	...
	private void dibujarfirmas(PDPageContentStream contentStream, float nextTextX, float nextTextY, Acta table) throws IOException {
		String footertitulotext = String.format("Responsabilidad por el manejo de bienes:");
		String footertext1 = String.format("Numero III art. 116 del DS 0181, todos los servidores publicos son responsables por el debido uso, custodia, preservacion y solicitud de servicios de");
		String footertext2 = String.format("mantenimiento de los bienes que les fueron asignados de acuerdo al regimen de Responsabilidad por la funcion Publica establecido en la Ley N° 1178");
		String footertext3 = String.format("y sus reglamentos.Asimismo el art. 157 numerales I.Los servidores publicos quedan prohibidos de a) Usar los bienes para benficio particular o privado.");
		String footertext4 = String.format("b) Permitir el uso para beneficio particular o privado. c) Prestar o transferir el bien a otro empleado publico. d) Enajenar el bien por cuenta propia");
		String footertext5 = String.format("e) Dañar o alterar sus caracteristicas fisicas o tecnicas f) Poner en riesgo el bien. g) Ingresar bienes particulares sin autorizacion de la Unidad");
		String footertext6 = String.format("Responsable de Activos Fijos. h) Sacar bienes de la entidad sin autoizacion de la Unidad o Responsable de Activos Fijos y II.La no observacion a estas");
		String footertext7 = String.format("prohibiciones generara responsabilidades establecidas en la Ley 1178 y sus reglamentos.");
		...
		contentStream.beginText();
		contentStream.moveTextPositionByAmount(nextTextX, nextTextY);
		contentStream.showText(footertext7);
		contentStream.endText();
	...
	//	nextTextY -= table.getRowHeight();
		...
		for (Firma firma : table.getFirmas()) {
			...
			contentStream.fillRect(nextTextX, nextTextY, (table.getWidth() - 2 * table.getMargin())/3 - 15, firma.getAlto() - 15);
