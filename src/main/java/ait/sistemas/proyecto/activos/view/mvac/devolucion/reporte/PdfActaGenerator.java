...
public class PdfActaGenerator {
	...
	private void dibujarfirmas(PDPageContentStream contentStream, float nextTextX, float nextTextY, Acta table)
		...
		String footertext3 = String
				.format("II.El servidor publico mientras se encuentre en instalaciones de la entidad publica prestando servicios sera responsable por el debido uso y custodia de los bienes a su cargo. III. La Unidad Administrativa ");
		
		String footertext4 = String.format("es responsable de ejecutar las acciones necesarias para proporcionar los mecanismos para asegurar la custodia de los bienes asignados a los servidores publicos.");
	...
	private void writeHeader(PDPageContentStream contentStream, float nextTextX, Acta table) throws IOException {
		...
		long text_width = (long) ((table.getTitleFont().getStringWidth(
				"Acta de Devolucion de Activos N°" + table.getNro_acta_entrega()) / 1000.0f) * 16);
		nextTextX = table.getWidth() / 2 - text_width / 2;
		contentStream.beginText();
		contentStream.moveTextPositionByAmount(nextTextX, nextTextY);

		contentStream.showText("Acta de Devolucion de Activos N°" + table.getNro_acta_entrega());
		contentStream.endText();
		...
		contentStream.beginText();
		contentStream.moveTextPositionByAmount(nextTextX, nextTextY);
		contentStream.showText("Devuelto por: ");
		contentStream.endText();
		contentStream.beginText();
		...
		contentStream.moveTextPositionByAmount(nextTextX, nextTextY);
		contentStream
				.showText("El Responsable de Activos Fijos hace constar por el presente documento que ha recibido a conformidad la devolucion de los activos que a continuacion se detallan: ");
		contentStream.endText();
		nextTextY -= 2 * table.getTb_activos().getRowHeight();
	}
