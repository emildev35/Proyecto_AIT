...
@CDIView(value = VInmuebleA.ID)
public class VInmuebleA extends VerticalLayout implements View, ClickListener {

	private static final long serialVersionUID = 1L;
	public static final String ID = "/act/rrhh/inmueble/a";
	...
	private final InmuebleImpl inmuble_impl = new InmuebleImpl();
	

	public VInmuebleA() {
		frm_inmueble = new FormInmueble();
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton() == this.btn_agregar) {
			if(this.frm_inmueble.validate()){
				this.inmuble_impl.add(this.frm_inmueble.getData());
				grid_inmueble.update();
				this.frm_inmueble.update();
				this.frm_inmueble.updateId();
