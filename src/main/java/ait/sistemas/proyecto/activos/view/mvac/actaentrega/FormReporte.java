...
//	private final MovimientoImpl movimiento_impl = new MovimientoImpl();
	public GridActaEntrega grid_Acta = new GridActaEntrega();
	public FormReporte() {
...
//		this.dt_fecha.setRequired(true);
		...
		this.txt_ci.addTextChangeListener(this);
		this.cb_funcionario.addValueChangeListener(this);
...
//	private void fillcbActas(String ci_funcionario) {
//		cb_actas.removeAllItems();
//		cb_actas.setNullSelectionAllowed(false);
//		for (Movimiento movimiento : movimiento_impl.getActaEbyFuncionario(ci_funcionario)) {
//			cb_actas.addItem(movimiento);
//			cb_actas.setItemCaption(
//					movimiento,
//					String.valueOf(movimiento.getNro_documento()));
//		}
//	}
	...
	private void buildContent() {
		...
		grid_fecha.addComponent(grid_Acta);
		
		Panel pn_form = new Panel("Actas de Entrega del Funcionario.Seleccione el Acta a imprimir ");
		pn_form.setIcon(FontAwesome.TABLE);
		pn_form.setStyleName(AitTheme.PANEL_GRID);
		pn_form.setContent(grid_fecha);

		vl_form.addComponent(pn_form);
		
		addComponent(pn_ci, 0, 0, 1, 0);
		addComponent(pn_dep, 0, 1, 2, 1);
		addComponent(pn_form, 0, 2, 2, 2);
	}
...
//		this.dt_fecha.setValue(new Date());
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (event.getProperty() == cb_funcionario && this.cb_funcionario.getValue() != null) {
			PersonalModel persona = (PersonalModel) cb_funcionario.getValue();
//			fillcbActas(persona.getPER_CI_Empleado());
			grid_Acta.update(persona.getPER_CI_Empleado());
		}
	}
...
//					if (activo != null) {
//						cb_actas.setValue(activo.getNombre());
//						fillcbActas(event.getText());
						grid_Acta.update(event.getText());
//					}
