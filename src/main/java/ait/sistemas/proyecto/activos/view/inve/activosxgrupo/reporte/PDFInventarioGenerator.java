package ait.sistemas.proyecto.activos.view.inve.activosxgrupo.reporte;
...
public class PDFInventarioGenerator {
	...
	public boolean generatePDF(Table table, String savePath, String grupo) throws IOException {
		grupoTitle = grupo;
	...
	public int saltopag(int r, Integer rowsPerPage, Table table, PDPageContentStream contentStream, float tableTopY,
		...
		drawCurrentPageOrden(table, new String[] { "(Orden:/Dependencia/Auxiliar/Codigo)" }, contentStream, tableTopY);
		r++;
	...
	public void drawTable(PDDocument doc, Table table) throws IOException {
		...
		double sum_CA_dependencia = 0;
		double sum_CA_por_dependencia = 0;
		double sum_CA_grupo = 0;
		double sum_CA_auxiliar = 0;
		
		double sum_DA_dependencia = 0;
		double sum_DA_por_dependencia = 0;
		double sum_DA_grupo = 0;
		double sum_DA_auxiliar = 0;
		...
		for (int i = 0; i < table.getContent().length; i++) {
			...
			if (!table.getContent()[i][2].equals(auxiliar_contable)) {
				...
				can_por_dependencia += can_auxiliares_contables;
				sum_por_dependencia += sum_auxiliares_contables;
				sum_neto_por_dependencia += sum_neto_auxiliares_contables;
				
				sum_CA_por_dependencia += sum_CA_auxiliar;
				sum_DA_por_dependencia += sum_DA_auxiliar;
				...
				if (!auxiliar_contable.equals("")) {
					...
					double ac_ca_total = sum_CA_auxiliar;
					String str_CA_total = formater.format(ac_ca_total);
					
					double da_total = sum_DA_auxiliar;
					String str_DA_total = formater.format(da_total);
					
					contentStream.drawLine(table.getMargin() * 21, tableTopY, table.getMargin() * 31, tableTopY);
					
					drawCurrentPageCorte(table, new String[] { "Cantidad items Auxiliar Contable", str_aux_cont, "Total",
							str_s_total, str_CA_total, str_DA_total, str_s_total_neto }, contentStream, tableTopY);
					if (r >= rowsPerPage || r == table.getHeaderSize()) {
						if (contentStream != null) {
							contentStream.close();
						}
				...
				if (r >= rowsPerPage || r == table.getHeaderSize()) {
					...
					if (!table.getContent()[i][0].equals(dependencia)) {
						...
						sum_CA_dependencia += sum_CA_grupo;
						sum_DA_dependencia += sum_DA_auxiliar;
						sum_CA_por_dependencia += sum_CA_grupo;
						sum_CA_grupo = 0;
						sum_DA_auxiliar = 0;
					}
				...
				if (!table.getContent()[i][1].equals(grupo_contable)) {
					...
					if (!grupo_contable.equals("")) {
						...
						drawCurrentPageCorte(table, new String[] { "Cantidad por Grupo Contable", str_grup_cont, "Total",
								str_s_grup_total, str_ca_grup_total, str_da_grup_total, str_s_grup_total_neto }, contentStream,
								tableTopY);
						// tableTopY -= table.getRowHeight();
						r++;
						if (r >= rowsPerPage || r == table.getHeaderSize()) {
							...
							drawCurrentPageDependencia(table, new String[] { "Dependencia :", dependencia }, contentStream,
									tableTopY);
							drawCurrentPageOrden(table, new String[] { "(Orden:/Dependencia/Auxiliar/Codigo)" }, contentStream,
									tableTopY);
					...
					can_dependencia += can_por_dependencia;
					sum_dependencia += sum_por_dependencia;
					sum_neto_dependencia += sum_por_dependencia;
					
					sum_CA_dependencia += sum_CA_por_dependencia;
					sum_DA_dependencia += sum_DA_por_dependencia;
					...
					sum_CA_por_dependencia += sum_CA_grupo;
					sum_DA_por_dependencia += sum_DA_grupo;
				...
				if (!table.getContent()[i][0].equals(dependencia)) {
					...
					double ca_dep_total = Double.parseDouble(String.valueOf(sum_CA_por_dependencia));
					String str_ca_dep_total = formater.format(ca_dep_total);
					
					double da_deptotal_neto = Double.parseDouble(String.valueOf(sum_DA_por_dependencia));
					String str_da_dep_total_neto = formater.format(da_deptotal_neto);
					
					tableTopY -= table.getRowHeight();
					contentStream.drawLine(table.getMargin() * 21, tableTopY, table.getMargin() * 31, tableTopY);
					
					drawCurrentPageCorte(table, new String[] { "Cantidad items Dependencia", str_f_dep_cont, "Total",
							str_f_dep_total, str_ca_dep_total, str_da_dep_total_neto, str_f_dep_total_neto }, contentStream,
							tableTopY);
					tableTopY -= table.getRowHeight();
					...
					can_dependencia += can_por_dependencia;
					sum_dependencia += sum_por_dependencia;
					sum_neto_dependencia += sum_neto_por_dependencia;
					...
					sum_CA_dependencia += sum_CA_por_dependencia;
					sum_DA_dependencia += sum_DA_por_dependencia;
					sum_CA_por_dependencia = 0;
					sum_DA_por_dependencia = 0;
					sum_CA_grupo += sum_CA_grupo;
					sum_DA_auxiliar = 0;
				...
				tableTopY -= table.getRowHeight();
				drawCurrentPageContable(table, new String[] { auxiliar_contable }, contentStream, tableTopY);
				r++;
				if (r >= rowsPerPage || r == table.getHeaderSize()) {
					if (contentStream != null) {
						contentStream.close();
			...
			sum_auxiliares_contables += Double.parseDouble(table.getContent()[i][7] == "null" ? "0" : table.getContent()[i][7]);
			sum_neto_auxiliares_contables += Double.parseDouble(table.getContent()[i][10] == "null" ? "0"
					: table.getContent()[i][10]);
			can_auxiliares_contables++;
			
			sum_CA_auxiliar += Double.parseDouble(table.getContent()[i][8] == "null" ? "0" : table.getContent()[i][8]);
			sum_DA_auxiliar += Double.parseDouble(table.getContent()[i][9] == "null" ? "0" : table.getContent()[i][9]);
			tableTopY -= table.getRowHeight();
		...
		contentStream.drawLine(table.getMargin() * 21, tableTopY, table.getMargin() * 31, tableTopY);
		drawCurrentPageCorte(table, new String[] { "Cantidad Items Auxiliar Contable", str_f_aux, "Total", str_f_aux_total,
				str_CA_total, str_DA_total, str_f_aux_total_neto }, contentStream, tableTopY);
		tableTopY -= table.getRowHeight();
		...
		contentStream.drawLine(table.getMargin() * 21, tableTopY, table.getMargin() * 31, tableTopY);
		drawCurrentPageCorte(table, new String[] { "Cantidad de Items Dependencia", str_f_dep_cont, "Total", str_f_dep_total,
				str_ca_dep_total, str_da_dep_total_neto, str_f_dep_total_neto }, contentStream, tableTopY);
		tableTopY -= table.getRowHeight();
		...
		if (numero_dependencias > 1) {
			...
			String str_ca_de_total = formater.format(sum_CA_dependencia);
			
			String str_da_de_total_neto = formater.format(sum_DA_dependencia);
			
			contentStream.drawLine(table.getMargin() * 21, tableTopY, table.getMargin() * 31, tableTopY);
			drawCurrentPageCorte(table, new String[] { "Cantidad de Items Inventario", str_f_sep_cont, "Total", str_f_sep_total,
					str_ca_de_total, str_da_de_total_neto, str_f_sep_total_neto }, contentStream, tableTopY);
			tableTopY -= table.getRowHeight();
		}
		contentStream.close();
	...
	private void drawTableGridGA(Table table, String[] strings, PDPageContentStream contentStream, float tableTopY)
		...
		// for (int i = 0; i <= currentPageContent.length + 1; i++) {
		for (int i = 0; i <= 1; i++) {
			contentStream.drawLine(table.getMargin(), nextY, table.getMargin() + table.getColumnsGA().get(0).getWidth(), nextY);
	...
	private void writeContentLine(String[] lineContent, PDPageContentStream contentStream, float nextTextX, float nextTextY,
		...
		for (int i = 0; i < lineContent.length; i++) {
			...
			if (i >= 3) {
				try {
					String text = new SimpleDateFormat("dd-MM-yyyy").format(format.parse(lineContent[i]));
					contentStream.beginText();
	...
	private void writeContentLineCorte(String[] lineContent, PDPageContentStream contentStream, float nextTextX, float nextTextY,
		...
		for (int i = 0; i < lineContent.length; i++) {
			...
			if (i == 3) {
				nextTextX = table.getMargin() + table.getWidth() - table.getColumns().get(4).getWidth()
						- table.getColumns().get(5).getWidth() - table.getColumns().get(6).getWidth();
			}
			if (i == 2) {
				nextTextX = table.getMargin() + table.getWidth() - table.getColumns().get(3).getWidth()
						- table.getColumns().get(4).getWidth() - table.getColumns().get(5).getWidth()
						- table.getColumns().get(6).getWidth();
			}
		}
	}
	...
	private void writeContentLineContables(String[] lineContent, PDPageContentStream contentStream, float nextTextX,
		...
		nextTextX = (float) (table.getColumns().get(1).getWidth() * 0.9);
		for (int i = 0; i < lineContent.length; i++) {
			String text = lineContent[i];
			contentStream.beginText();
			contentStream.moveTextPositionByAmount(nextTextX, nextTextY);
			contentStream.showText(text != null ? text : "");
			contentStream.endText();
			nextTextX += table.getColumns().get(2).getWidth() * 0.86;
	...
	private void writeHeader(PDPageContentStream contentStream, float nextTextY, Table table) throws IOException {
		...
		contentStream.beginText();
		text_width = (long) ((table.getSubtitleFont().getStringWidth(grupoTitle) / 1000.0f) * table.getFontSizesubtitle());
		contentStream.moveTextPositionByAmount((table.getWidth() / 2) - (text_width / 2) + (table.getMargin() / 2), nextTextY);
		contentStream.showText(grupoTitle);
		contentStream.endText();
