package ait.sistemas.proyecto.activos.view.reva.resumenact.reporte;
...
public class ResumenActuGenerator {
	...
	public void drawTable(PDDocument doc, Table table) throws IOException {
		...
		DecimalFormat formaterint = new DecimalFormat("##,###,###,##0");
		
		int can_dependencia = 0;
		double sum_valorCompra = 0;
		double sum_valorNeto = 0;
		double dep_anterior = 0;
		double depActual = 0;
		double actAnterior = 0;
		double actActual = 0;
		double actAcumulada = 0;
		...
		for (int i = 0; i < table.getContent().length; i++) {
			...
			if (!table.getContent()[i][0].equals(dependencia) && contentStream != null) {
				...
				drawCurrentPageCorte(
						...
						new String[] { num_dep > 1 ? "Total General" : "Total Dependencia", formaterint.format(can_dependencia),
								formater.format(sum_valorCompra), formater.format(actAnterior), formater.format(dep_anterior),
								formater.format(actActual), formater.format(depActual), formater.format(actAcumulada),
								formater.format(depAcumulada), formater.format(sum_valorNeto)
			...
			can_dependencia += Integer.parseInt(table.getContent()[i][2]);
			sum_valorCompra += Double.parseDouble(table.getContent()[i][3]);
			actAnterior += Double.parseDouble(table.getContent()[i][4]);
			dep_anterior += Double.parseDouble(table.getContent()[i][5]);
			actActual += Double.parseDouble(table.getContent()[i][6]);
			depActual += Double.parseDouble(table.getContent()[i][7]);
			actAcumulada += Double.parseDouble(table.getContent()[i][8]);
			depAcumulada += Double.parseDouble(table.getContent()[i][9]);
			sum_valorNeto += Double.parseDouble(table.getContent()[i][10]);
		...
		drawCurrentPageCorte(
				...
				new String[] { num_dep > 1 ? "Total General" : "Total Dependencia", formaterint.format(can_dependencia),
						formater.format(sum_valorCompra), formater.format(actAnterior), formater.format(dep_anterior),
						formater.format(actActual), formater.format(depActual), formater.format(actAcumulada),
						formater.format(depAcumulada), formater.format(sum_valorNeto) }, contentStream, tableTopY);
	...
	private void writeContentLineHeader(String[] lineContent, PDPageContentStream contentStream, float nextTextX,
		...
		for (int i = 0; i < lineContent.length; i++) {
			...
			while (st.hasMoreTokens()) {
				...
				contentStream.endText();
				copynextTextY -= 0.75 * table.getRowHeight();
			}
	...
	private void writeHeader(PDPageContentStream contentStream, float nextTextY, Table table) throws IOException {
		...
		long size_header = table.isLandscape() ? 850 : 450;
		Date date = new Date();
