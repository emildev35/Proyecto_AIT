...
@CDIView(value = VTipoSoporteA.ID)
public class VTipoSoporteA extends VerticalLayout implements View, ClickListener {

	private static final long serialVersionUID = 1L;
	public static final String ID = "/act/para/tiposoporte/a";
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton() == this.btn_agregar) {
			if(this.frm_tipo_soporte.validate()){
				if (this.tipo_soporteimpl.agregar(this.frm_tipo_soporte.getData())>0) {
					grid_tipo_soporte.update();
					this.frm_tipo_soporte.update();
					this.frm_tipo_soporte.updateId();
