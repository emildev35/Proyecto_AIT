...
import ait.sistemas.proyecto.common.report.PDFInventarioSinAsigGenerator;
...
public class ReportPdf {
	...
	public boolean getPdf(String[][] data, String strDependencia, String fecha) throws IOException {
		...
		this.SAVE_PATH = PathValues.PATH_REPORTS + "Reporte-Inventariosinasig-" + String.valueOf(new java.util.Date().getTime())
				+ ".pdf";
		return new PDFInventarioSinAsigGenerator().generatePDF(createContent(data, strDependencia, fecha), SAVE_PATH);
	...
	private Table createContent(String[][] content, String strDependencia, String fecha) {
		...
		columns.add(new Column("Nombre del Activo", 495));
		columns.add(new Column("Valor Compra", 75));
		columns.add(new Column("Valor Neto", 75));
		columns.add(new Column("Vida Residual", 75));
		...
		Table Inventario = new TableBuilder().setCellMargin(CELL_MARGIN).setColumnsGA(columnsGA).setContent(content)
				...
				.setTitle("INVENTARIO DE ACTIVOS SIN ASIGNAR").setSubTitle("Realizado al : " + fecha).build();
		return Inventario;
