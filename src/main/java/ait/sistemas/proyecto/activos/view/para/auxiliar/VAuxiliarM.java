...
@CDIView(value = VAuxiliarM.ID)
public class VAuxiliarM extends VerticalLayout implements View, ClickListener,
		SelectionListener {

	private static final long serialVersionUID = 1L;
	public static final String ID = "/act/para/auxiliar/m";
	...
	public VAuxiliarM() {
		...
		this.frm_auxiliar.txt_id_auxiliar.setEnabled(false);
		this.frm_auxiliar.cb_grupo.setEnabled(false);
	...
	private Component buildNavBar() {
		...
		nav.addComponent(new Label("Auxilaires Contables>>"));
	...
	public void select(SelectionEvent event) {
		...
		if ((AuxiliaresContablesModel) this.grid_auxiliar.getSelectedRow() != null) {
			this.frm_auxiliar.setData((AuxiliaresContablesModel) this.grid_auxiliar
					.getSelectedRow());
		}
	}
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton() == this.btn_modificar) {
			if (this.frm_auxiliar.validate()) {
				this.auxiliar_impl.update(this.frm_auxiliar.getData());
