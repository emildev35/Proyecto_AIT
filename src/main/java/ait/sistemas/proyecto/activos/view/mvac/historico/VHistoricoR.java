...
public class VHistoricoR extends VerticalLayout implements View, ClickListener {
	
	private static final long serialVersionUID = 1L;
	
	private Button btn_imprimir;
	private FormHistorico frmReporte = new FormHistorico();
	int r = 0;
	...
	public HisotricoElement[][][] getData(List<ActivosModel> lista) throws SQLException {
...
//			}
			...
			kardex_elements[r][0][1] = new HisotricoElement();
			...
			kardex_elements[r][0][2] = new HisotricoElement();
...
//				SessionModel session = (SessionModel) UI.getCurrent().getSession().getAttribute("user");
				...
				if (frmReporte.getActivo().equals(a)) {
					List<ActivosModel> data = activo_impl.ActivosbyAuxiliarFecha(
							frmReporte.cb_Auxiliar.getValue().toString(),
							frmReporte.cb_Grupo.getValue().toString(),new java.sql.Date(frmReporte.dtf_fechaElaboracion.getValue().getTime())
							);
					reporte.getPdfKardexGeneratorMulti(getData(data), frmReporte.cb_Activos.getValue(),
							frmReporte.dtf_fechaElaboracion.getValue());
				} else {
					List<ActivosModel> data = activo_impl.getActivosbyCodigoFecha(Long.parseLong(frmReporte.getActivo()), new java.sql.Date(frmReporte.dtf_fechaElaboracion.getValue().getTime()));
					reporte.getPdf(getData(data)[0], frmReporte.cb_Activos.getValue(),
							frmReporte.dtf_fechaElaboracion.getValue());
				}
