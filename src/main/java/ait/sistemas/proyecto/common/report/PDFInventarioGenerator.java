	...
	public void drawTable(PDDocument doc, Table table) throws IOException {
		...
		for (int i = 0; i < table.getContent().length; i++) {
			...
			sum_auxiliares_contables += Float
					.parseFloat(table.getContent()[i][6].replace(".", "").replace(",", ".") == "null" ? "0" : table
							.getContent()[i][6].replace(".", "").replace(",", "."));
			sum_neto_auxiliares_contables += Float.parseFloat(table.getContent()[i][7].replace(".", "").replace(",",
					".") == "null" ? "0" : table.getContent()[i][7].replace(".", "").replace(",", "."));
