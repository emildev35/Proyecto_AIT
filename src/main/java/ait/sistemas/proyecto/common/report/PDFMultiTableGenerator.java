...
public class PDFMultiTableGenerator {
	...
	public boolean generatePDF(Table[] table, String savePath)
			throws IOException {
	...
	public void drawTable(PDDocument doc, Table table[]) throws IOException {
		for (int i = 0; i < table.length; i++) {
			...
			Integer rowsPerPage = new Double(Math.floor(table[i].getHeight()
					/ table[i].getRowHeight())).intValue() - 1;
			Integer numberOfPages = new Double(Math.ceil((table[i]
					.getNumberOfRows().floatValue() + table[i].getHeaderSize())
			...
			for (int pageCount = 0; pageCount < numberOfPages; pageCount++) {
				...
				currentPageContent = getContentForCurrentPage(table[i],
						rowsPerPage, pageCount);
				drawCurrentPage(table[i], currentPageContent, contentStream,
						pageCount);
			}
		}
	}
