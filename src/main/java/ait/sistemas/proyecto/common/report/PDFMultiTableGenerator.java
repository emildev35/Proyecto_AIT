...
public class PDFMultiTableGenerator {
	...
	public void drawTable(PDDocument doc, Table table[]) throws IOException {
		for (int i = 0; i < table.length; i++) {
			Integer rowsPerPage = new Double(Math.floor(table[i].getHeight() / table[i].getRowHeight()))
					.intValue() - 1;
			Integer numberOfPages = new Double(Math.ceil((table[i].getNumberOfRows().floatValue() + table[i]
					.getHeaderSize()) / rowsPerPage)).intValue();
			this.intNumberoftotalPages += numberOfPages;
		}
		for (int i = 0; i < table.length; i++) {
			Integer rowsPerPage = new Double(Math.floor(table[i].getHeight() / table[i].getRowHeight()))
					.intValue() - 1;
			...
			for (int pageCount = 0; pageCount < numberOfPages; pageCount++) {
				this.intactualpage++;
				PDPage page = generatePage(doc, table[i]);
				PDPageContentStream contentStream = generateContentStream(doc, page, table[i]);
				String[][] currentPageContent;
				currentPageContent = getContentForCurrentPage(table[i], rowsPerPage, pageCount);
				drawCurrentPage(table[i], currentPageContent, contentStream, pageCount);
			}
		}
	}
	...
	private void drawCurrentPage(Table table, String[][] currentPageContent,
			PDPageContentStream contentStream, int pageCount) throws IOException {
		float tableTopY;
		...
		if (pageCount == 0) {
			tableTopY = table.isLandscape() ? table.getPageSize().getWidth() - table.getMargin()
					...
					- table.getMargin() - table.getRowHeight()
	...
	private void writeFooter(PDPageContentStream contentStream, float nextTextX, float nextTextY,
			Table table, int pagecount) throws IOException {
		...
		contentStream.drawLine(nextTextX, (nextTextY + table.getRowHeight()),
				nextTextX * table.getNumberOfColumns(), (nextTextY + table.getRowHeight()));
	...
	private void writeHeader(PDPageContentStream contentStream, float nextTextX, float nextTextY, Table table)
		...
		long text_width = (long) ((table.getTitleFont().getStringWidth(table.getTitle()) / 1000.0f) * table
				.getFontSizetitle());
		contentStream.moveTextPositionByAmount((table.getWidth() / 2) - (text_width / 2)
				+ (table.getMargin() / 2), nextTextY);
		...
		long sub_width = (long) ((table.getSubtitleFont().getStringWidth(table.getSubtitle()) / 1000.0f) * table
				.getFontSizesubtitle());
		contentStream.moveTextPositionByAmount((table.getWidth() / 2) - (sub_width / 2)
				+ (table.getMargin() / 2), nextTextY);
