...
@CDIView(value=VSubSistemaB.ID)
public class VSubSistemaB extends VerticalLayout implements View,
		SelectionListener, ClickListener, org.vaadin.dialogs.ConfirmDialog.Listener {
	public static final String ID = "/seg/estr/subsistema/b";
	...
	public VSubSistemaB() {
		...
		pitmSubSistema.addItemProperty("identificador",
				new ObjectProperty<Integer>(0));
		pitmSubSistema.addItemProperty("id_subsistema",
				new ObjectProperty<Integer>(0));
		pitmSubSistema.addItemProperty("nombre_subsistema",
				new ObjectProperty<String>(""));
		pitmSubSistema.addItemProperty("nombre_programa",
				new ObjectProperty<String>(""));
		pitmSubSistema
				.addItemProperty("iconos", new ObjectProperty<String>(""));
	...
	private void eliminar() {
		try {
			this.menuImpl
					.deleteSubsistema(subsistema.getAME_Id_Identificador());
			...
			buildError(Messages.FOREIGN_RELATION_ERROR(subsistema
					.getAME_Nombre()));
		}
	}
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton() == this.btn_submit) {
			ConfirmDialog.show(getUI(), "Eliminar " + this.subsistema.getAME_Nombre(),
					"Esta Uds Seguro que Desea Eliminar el Sub-Sistema" + this.subsistema.getAME_Nombre(),
					"Si", "No", this);	
		}
	...
	public void select(SelectionEvent event) {
		...
		if ((Arbol_menus) this.gridSubSistemas.getSelectedRow() != null) {
			this.subsistema = (Arbol_menus) this.gridSubSistemas
					.getSelectedRow();
			this.txt_identificador.setValue(subsistema
