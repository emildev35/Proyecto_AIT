...
public class FormUsuario extends GridLayout implements ValueChangeListener {
	...
	public TextField txtIdenticadorUsuario = new TextField(
			"Identicador Usuario");
	...
	public FormUsuario() {
		...
		pitmUsuario.addItemProperty("personal",
				new ObjectProperty<PersonalModel>(new PersonalModel()));
		pitmUsuario.addItemProperty("ci", new ObjectProperty<String>(""));
		pitmUsuario.addItemProperty("identificador",
		...
		this.txtIdenticadorUsuario.addValidator(new StringLengthValidator(
				Messages.STRING_LENGTH_MESSAGE(10, 12), 10, 12, false));
	...
	private void fullcbUsuario() {
		...
		for (PersonalModel personal : personalimpl.getalls()) {
			...
			String strNombreCompleto = String.format("%s %s %s",
	...
	private void fillData(PersonalModel value) {
		...
		String strIdentificador = String.format("%s-%s-%s",
				value.getPER_Nombres().substring(0, 2),
	...
	public boolean validate() {
		...
		try {
			...
			try {
				...
				this.mesagges.add(new BarMessage(cbPersonal.getCaption(),
						Messages.EMPTY_MESSAGE));
			}
			try {
				this.txtIdenticadorUsuario.validate();
			} catch (EmptyValueException e2) {
				this.mesagges.add(new BarMessage(txtIdenticadorUsuario
						.getCaption(), Messages.EMPTY_MESSAGE));
			} catch (InvalidValueException ev) {
				this.mesagges.add(new BarMessage(txtIdenticadorUsuario
						.getCaption(), ev.getMessage()));
			}
			return false;
		}
