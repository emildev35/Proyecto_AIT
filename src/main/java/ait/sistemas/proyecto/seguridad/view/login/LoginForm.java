...
public class LoginForm extends GridLayout implements TextChangeListener {
	...
	public PasswordField pwd_password_verification = new PasswordField("Confirme Contraseña");
	...
	public LoginForm() {
		...
		this.txt_id_usuario.addValidator(new StringLengthValidator(Messages.STRING_LENGTH_MESSAGE(5, 12), 5, 12, false));
		...
		try {
			...
			if (getComponent(0, 2) != null) {
				...
				if (!this.pwd_password.getValue().equals(this.pwd_password_verification.getValue())) {
					this.mensajes
							.add(new BarMessage(this.pwd_password_verification.getCaption(), Messages.ERROR_EQUALS_PASSWORD));
					return false;
				}
			...
			try {
				...
				this.mensajes.add(new BarMessage(this.txt_id_usuario.getCaption(), Messages.EMPTY_MESSAGE));
			} catch (InvalidValueException sle) {
				this.mensajes.add(new BarMessage(this.txt_id_usuario.getCaption(), sle.getMessage()));
			}
			try {
				this.pwd_password.validate();
				
			} catch (EmptyValueException eve) {
				this.mensajes.add(new BarMessage(this.pwd_password.getCaption(), Messages.EMPTY_MESSAGE));
			} catch (InvalidValueException sle) {
				this.mensajes.add(new BarMessage(this.pwd_password.getCaption(), sle.getMessage()));
			}
